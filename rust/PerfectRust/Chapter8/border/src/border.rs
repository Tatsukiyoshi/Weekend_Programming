...
impl<T> Customer<T> where T:std::ops::Add {
    /// ## 8-6.ジェネリックス
    /// ### リスト8.17 ジェネリック型の利用
...
pub fn use_change_id(){
    ...
    println!("{:?}", customer);
    customer.change_id(300);
    println!("{:?}", customer);

    /* Error: "String: Add was not satisfied"
    let mut customer = Customer::<String>::new(String::from("100"), String::from("山田太郎"),
        String::from("東京都新宿区"), String::from("yamada@sample.com"));
    println!("{:?}", customer);
    customer.change_id(String::from("300"));
    println!("{:?}", customer);
    */
}
