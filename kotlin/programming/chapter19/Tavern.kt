// チャレンジ：関数型プログラミングをTavern.ktに応用
package com.programming.kotlin.chapter19
...
val uniquePatrons: MutableSet<String> = generateSequence {
    patronList.shuffled().first() + " " + lastName.shuffled().first()
}.distinct()
  .take(9)
  .toMutableSet()
...
fun performPurchase(price: Double, patronName: String) {
    ...
    // お金が足りない人は、リストから削除
    if (totalPurse < price) {
        // <Amendment04>mistype(says -> saye)
        println("The tavern master says: Get out! $patronName.")
...
private fun showFormattedMenuLine(_type: String, _name: String, _price:String) {
    ...
    //「パラメータが未使用」という警告に対して、参照するコードを追加する
    if (_type == "") {
        return
    }
